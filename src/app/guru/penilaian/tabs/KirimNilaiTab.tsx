...
// Isi tab "Kirim Nilai" di menu Penilaian (/guru/penilaian). Sebelumnya
// halaman sendiri (/guru/kirim-nilai) — logikanya tidak diubah, cuma
// dipindah jadi salah satu tab.
// `focusTarget` dipakai untuk jembatan dari tab "Periksa Jawaban Essay":
// begitu guru merilis semua nilai essay satu sesi di tab itu dan menekan
// "Lanjut ke Kirim Nilai", grup mapel+kelas yang sama otomatis terbuka di
...
export function KirimNilaiTab({
  ...
  // Jembatan dari tab "Periksa Jawaban Essay" (lihat komentar di atas) —
  // buka grup mapel+kelas yang barusan selesai dirilis, supaya guru
  // langsung lihat tombol "Kirim Semua ke Wali Kelas" untuk grup itu.
  useEffect(() => {
    if (focusTarget) setExpandedGroup(`${focusTarget.mapelId}__${focusTarget.kelas}`)
  }, [focusTarget])
                      ...
                      return (
                        <div key={sesiId} className="px-5 py-4 bg-indigo-50/40">
                          ...
                          <p className="text-xs text-indigo-400 mb-2">
                            Sama dengan tombol rilis di tab Periksa Jawaban Essay — cukup dari salah satu.
                          </p>
