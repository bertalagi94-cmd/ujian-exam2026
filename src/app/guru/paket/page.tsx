        ...
        <div className="space-y-3">
          ...
          {pakets.map(p => (
            <div key={p.id} className="card p-0 overflow-hidden">
              <div className="flex flex-col lg:flex-row lg:items-center gap-2.5 lg:gap-4 p-3 lg:p-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 lg:gap-2 flex-wrap">
                    <span className="font-semibold text-sm lg:text-base text-slate-900">{p.nama_mapel ?? getNamaMapel(p.mapel_id)}</span>
                    <span className="text-slate-400 text-xs">·</span>
                    <span className="text-xs lg:text-sm text-slate-600">Kelas {p.nama_kelas ?? getNamaKelas(p.kelas_id)}</span>
                  ...
                  {p.status === 'DITOLAK' && (
                    <div className="mt-1 text-[11px] lg:text-xs text-red-600">Soal ditolak — silakan edit soal lalu kirim ulang</div>
                  )}
                </div>
                <div className="flex items-center gap-1.5 lg:gap-2 flex-shrink-0 flex-wrap justify-start lg:justify-end">
                  {p.status === 'DRAFT' && (
                    <button onClick={() => lanjutkanPaket(p)} className="btn-secondary !px-2 !py-1 !text-[11px] lg:!px-3 lg:!py-1.5 lg:!text-xs !rounded-lg">
                      <Plus className="w-3 h-3 lg:w-3.5 lg:h-3.5" /> Lanjutkan
                    </button>
                  )}

                  {/* Duplicate — selalu tersedia */}
                  <button onClick={() => { setDupId(p.id); setDupKelas('') }}
                    className="btn-secondary !px-2 !py-1 !text-[11px] lg:!px-3 lg:!py-1.5 lg:!text-xs !rounded-lg" title="Duplikasi ke kelas lain">
                    <Copy className="w-3 h-3 lg:w-3.5 lg:h-3.5" /> Duplikasi
                  </button>

                  {/* Kirim */}
                  {(p.status === 'DRAFT' || p.status === 'DITOLAK') && (
                    <button onClick={() => bukaKirim(p.id)} className="btn-primary !px-2 !py-1 !text-[11px] lg:!px-3 lg:!py-1.5 lg:!text-xs !rounded-lg">
                  ...
                  {(p.status === 'DRAFT' || p.status === 'DITOLAK') && (
                    <button
                      ...
                      className="btn-ghost btn-icon !p-1.5 lg:!p-2 !rounded-lg lg:!rounded-xl text-red-500 hover:bg-red-50"
                  ...
                  {/* Tarik */}
                  {p.status === 'MENUNGGU' && (
                    <button onClick={() => setTarikId(p.id)} className="btn-secondary !px-2 !py-1 !text-[11px] lg:!px-3 lg:!py-1.5 lg:!text-xs !rounded-lg">
                      <RotateCcw className="w-3 h-3 lg:w-3.5 lg:h-3.5" /> Tarik
                    </button>
                  )}
                  ...
                  <button
                    ...
                    className="btn-ghost btn-icon !p-1.5 lg:!p-2 !rounded-lg lg:!rounded-xl"
                  >
                    {expandedId === p.id ? <ChevronUp className="w-3.5 h-3.5 lg:w-4 lg:h-4" /> : <ChevronDown className="w-3.5 h-3.5 lg:w-4 lg:h-4" />}
                  </button>
                </div>
              </div>

              {p.status === 'MENUNGGU' && p.catatan && (
                <div className="px-3 lg:px-4 pb-3 -mt-1 lg:-mt-2 text-[11px] lg:text-xs text-amber-600">Persetujuan dibatalkan admin — klik <strong>Tarik</strong> untuk mengedit soal lalu kirim ulang</div>
              )}
            ...
            <div key={p.id} className="card p-0 overflow-hidden">
              <div className="flex flex-col lg:flex-row lg:items-center gap-2.5 lg:gap-4 p-3 lg:p-4">
                <div className="flex-1 min-w-0">
                  ...
                  {p.status === 'MENUNGGU' && p.catatan && (
                    <div className="mt-1 text-[11px] lg:text-xs text-slate-500">Menunggu validasi admin</div>
                  )}
                </div>
                <div className="flex items-center gap-1.5 lg:gap-2 flex-shrink-0 flex-wrap justify-start lg:justify-end">
